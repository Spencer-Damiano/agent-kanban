...
- **Auth model**: how human vs. agent credentials are issued and distinguished. The state machine's permission rules depend on the API deriving actor type from the credential, never from the request body. *Interim (until roadmap item 4): callers self-identify with `x-actor-type` / `x-actor-id` headers, resolved in `src/routes/actor.ts` so real auth swaps one module.*
